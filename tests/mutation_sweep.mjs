...
const MUTATIONS = [
  ...
  ["1.3 containment: the drawer why-fit heading element returns",
    '        <div class="drawer-section-label" id="drawerProofsLabel">What to notice</div>',
  ...
  ["product-proof: the finalist change is no longer announced",
    ...
    "      live.textContent = '';",
    A31],
  ["A3.1 drawer: the second trial prompt returns (the same trial explained twice)",
    "      var trialCue = L(m.trialCue || '') || getMattressTrialPrompts(m)[0] || '';",
    "      var trialCue = getMattressTrialPrompts(m).slice(0, 2).join(' ');",
    A31],
  // --- Product-proof drawer, slice 3 (owner-approved revision 2026-09-02) ----
  ["product-proof: the canonical trial cue is ignored (engine prompt always)",
    "      var trialCue = L(m.trialCue || '') || getMattressTrialPrompts(m)[0] || '';",
    "      var trialCue = getMattressTrialPrompts(m)[0] || '';",
    A31],
  ...
  ["product-proof: a third proof renders",
    ...
    A31],
  ["product-proof: the proof label stays visible with zero proofs",
    "      if (proofBlock) proofBlock.hidden = proofs.length === 0;",
    "      if (proofBlock) proofBlock.hidden = false;",
    A31],
  ["product-proof: a construction label is invented from quiz features",
    ...
    A31],
  ["product-proof: a support row renders when the story names no support layer",
    "      if (labels.support) rows.push([roleTerms[1], labels.support]);",
    "      rows.push([roleTerms[1], labels.support]);",
    A31],
  ["product-proof: the inside block never collapses",
    "      if (insideBlock) insideBlock.hidden = !(rows.length || schematic);",
    "      if (insideBlock) insideBlock.hidden = false;",
    A31],
