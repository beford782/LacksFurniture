...
const MUTATIONS = [
  ...
  // A3.1 (owner directive 2026-09-01): names + ordinal ring on the Summary.
  ["A3.1: the hf2 render restores the reason prose beside the name",
    "          + '<strong>' + escapeHtml(L(item)) + '</strong>'\n          + '</li>';",
    "          + '<strong>' + escapeHtml(L(item)) + '</strong> \u2014 ' + escapeHtml(item.why[currentLang] || item.why.en)\n          + '</li>';",
    PRIORITIES],
  ["A3.1: the hf2 render drops the ordinal ring",
    "          + '<span class=\"hf2-ordinal\" aria-hidden=\"true\">' + (i + 1) + '</span>'\n",
    "", PRIORITIES],
  ...
  ["cons 5b: markup varies by model (generic boundary broken)",
    ...
    "        '<div class=\"dfm-cons\" id=\"dfmConstructionPanel\" data-model=\"' + (((window.currentDrawerMattress || {}).id) || '') + '\">' +",
    CONS],
  ...
  // X11 (North Star D9, 2026-08-31): forced-colors state cues by geometry.
  ["X11: the forced active tab regresses to a 2px solid cue (identical to the forced resting border)",
    "      .noct-tier-tab.active { border: 3px double CanvasText; padding: 9px 19px 9px 23px; }",
    "      .noct-tier-tab.active { border: 2px solid CanvasText; }", RESULTS.concat(["tests/contrast_check.mjs"])],
  ["X11: the forced active tab loses its padding compensation (label shifts on a tab switch)",
    "      .noct-tier-tab.active { border: 3px double CanvasText; padding: 9px 19px 9px 23px; }",
    "      .noct-tier-tab.active { border: 3px double CanvasText; }", RESULTS.concat(["tests/contrast_check.mjs"])],
  ["X11: the rail-step and card-control forced cues are dropped",
    "    @media (forced-colors: active) {\n      .sleep-system__step.is-active {\n        border-width: 3px;\n        border-color: CanvasText;\n        padding: 8px 10px;\n      }\n      #resultsScreen .compare-btn.selected,\n      #resultsScreen .noct-save-btn.saved {\n        border-width: 2px;\n        border-color: CanvasText;\n        padding: 6.5px 12.5px;\n      }\n    }\n",
    "", ["tests/contrast_check.mjs"]],
  ["X11: the selected card control's forced cue loses its width (colour-only again)",
    "        border-width: 2px;\n        border-color: CanvasText;\n        padding: 6.5px 12.5px;",
    "        border-color: CanvasText;\n        padding: 8px 14px;", ["tests/contrast_check.mjs"]],
  // X12 (North Star D9, 2026-08-31): the 44px touch floor.
  ["X12: the Selections pill drops back under the 44px floor",
    "      min-height: 44px; /* X12: 44px touch floor (was 40px) */\n",
    "", RESULTS],
  ["X12: the compare tray's Clear / Compare drop back to 35px",
    ".compare-tray-clear, .compare-tray-go { white-space: nowrap; min-height: 44px; min-width: 44px; }",
    ".compare-tray-clear, .compare-tray-go { white-space: nowrap; }", RESULTS],
  ["X12: the Welcome language toggle drops back to 40x27",
    "      min-height: 44px;\n      min-width: 44px;\n      font-family: var(--font-sans);\n      font-size: 11px;\n      font-weight: 500;\n      letter-spacing: 0.1em;",
    "      font-family: var(--font-sans);\n      font-size: 11px;\n      font-weight: 500;\n      letter-spacing: 0.1em;", RESULTS],
  ["X12: the Compare close control loses its author focus ring",
    "    .compare-modal-close:focus-visible {\n      outline: 3px solid var(--focus-ring-outer);\n      outline-offset: 2px;\n      box-shadow: 0 0 0 5px var(--focus-ring-inner);\n    }\n",
    "", RESULTS],
  ["X12: a Summary pick action returns to the 42px floor",
    "    .hf2-pick__action {\n      min-height: 44px; /* X12: 44px touch floor */",
    "    .hf2-pick__action {\n      min-height: 42px;", RESULTS],
  // Wave 3 / X4 (North Star D1, 2026-08-31): the banner fallback.
  ["Wave 3: the banner load listener is dropped (no image ever tagged)",
    "    document.addEventListener('load', function(e) {\n      dfTagBannerImage(e.target);\n    }, true);\n",
    "", RESULTS],
  ["Wave 3: the banner threshold is raised so no real source ever qualifies",
    "    var DF_BANNER_AR = 3;",
    "    var DF_BANNER_AR = 30;", RESULTS],
  ["Wave 3: the fallback loses to the drawer hero's inline cover (no !important)",
    "      object-fit: contain !important;\n      background: #FFFDF8;",
    "      object-fit: contain;\n      background: #FFFDF8;", RESULTS],
  // X2/X3/X10 (North Star ruling D7, 2026-08-31): honest Sleep System states,
  // factual wording, and the keyboard's place.
  ["X2: a deferral counts as addressed again ('Decide later' claimed as done)",
    "      if (status === 'later') return 'deferred';",
    "      if (status === 'later') return 'addressed';", SLEEP],
  ...
  ["X10: the re-render stops restoring the keyboard's place",
    "      var focusKey = sleepSystemFocusKey();\n      renderSleepSystemRail();",
    "      renderSleepSystemRail();", SLEEP],
  ["X10: the active rail step loses aria-current",
  ...
  ["X8: the focus hand-off regresses (showScreen's announce path stays suppressed during the swap)",
    "          window._resultsRevealInFlight = false;\n          window.showResults();\n          window._resultsRevealInFlight = true;",
    "          window.showResults();", MOTION],
  ["X9: the decode hold is dropped (the mask lifts onto a half-painted hero)",
    "        var hero = document.querySelector('#topPickContainer img.noct-toppick-photo');\n        if (hero && typeof hero.decode === 'function') {\n          hero.decode().then(lift, lift);\n        } else {\n          lift();\n        }",
    "        lift();", MOTION],
  ["X9: the 400ms cap is removed (a slow image holds the mask forever)",
    "        sessionTimeout(lift, 400);\n",
    "", MOTION],
  ...
  ["plan: a ninth .screen container is added and registered nowhere",
    '  <div class="screen" id="sleepPlanScreen" role="region" data-audience="shared">',
    '  <div class="screen" id="ghostScreen" role="region"></div>\n  <div class="screen" id="sleepPlanScreen" role="region" data-audience="shared">', SESSION],
  ...
  ["slice6: the Summary retitles back to the Plan's name",
    "        hf2ReviewTitle: es ? 'Resumen de la consulta' : 'Consultation summary',",
  ...
  // A3.1 (owner directive 2026-09-01): the note panel is retired; reviving it
  // with a note in the aside must trip the retired-panel pins.
  ["A3.1: the retired note panel is revived in the aside",
    "      guidance.hidden = !financingBlock;",
    "      guidance.hidden = false; guidance.innerHTML = '<div class=\"sleep-system__notice\"><span>&#10003;</span><span>note</span></div>' + financingBlock;",
    SLEEP],
  // (7) the retained aligned note back in customer voice.
  ["1.4 close-out/A3.1: the retained pillow technique line reverts to customer voice",
    "          en: 'Test it on the finalist mattress and watch the neck line from the side.',",
    "          en: 'Test it on your finalist mattress and watch your neck line from the side.',",
    SLEEP],
  // (8) an ES counterpart goes English-only (the retained adjustability note).
  ["1.4 close-out/A3.1: a retained line's ES value is replaced by its EN value",
    "es: 'Si importa una altura de cama m\u00e1s baja, pregunta qu\u00e9 alturas de base est\u00e1n disponibles.'",
    "es: 'If a lower finished bed height matters, ask which foundation heights are available.'",
    SLEEP],
  ...
  ["A3.1 ruling 5: the Summary still-open list drops the specialist-check state",
    ...
    A31],
  ["A3.1 synthesis: the fit-response mark loses aria-hidden (a decorative silhouette reaches assistive technology)",
    "      return '<span class=\"sleep-system__fit-response\" data-fit-response=\"' + id + '\" aria-hidden=\"true\">' +",
    "      return '<span class=\"sleep-system__fit-response\" data-fit-response=\"' + id + '\">' +",
    A31],
  ["A3.1 synthesis: every fit-response mark collapses to the aligned silhouette (the mark stops answering the response)",
    "      var headY = id === 'low' ? 15 : (id === 'high' ? 5 : 10);",
    "      var headY = 10;",
    A31],
  ["A3.1 synthesis: the fit-response note is dropped (the marks could read as the pillow's own height)",
    "        '<p class=\"sleep-system__fit-response-note\">' +",
    "        '<p class=\"sleep-system__fit-response-note\" hidden>' +",
    A31],
  ["A3.1 synthesis: the goal glyph loses aria-hidden (a decorative glyph reaches assistive technology)",
    "      return '<span class=\"sleep-system__goal-glyph\" data-goal-glyph=\"' + id + '\" aria-hidden=\"true\">' +",
    "      return '<span class=\"sleep-system__goal-glyph\" data-goal-glyph=\"' + id + '\">' +",
    A31],
  ["A3.1 synthesis: every goal glyph collapses to the everyday calendar (the glyph stops answering the goal)",
    "      var shape = shapes[id] || shapes.everyday;",
    "      var shape = shapes.everyday;",
    A31],
  ["A3.1 synthesis: the goal glyph replaces the governed copy line (text pairing lost)",
  ...
  ["A3.1 synthesis: the Summary's two lists share one concept name again (Visit focus label reverts to Priorities)",
    ...
    A31],
  ["A3.1 synthesis (ruling 6): the confirmation regrows the config suffix about connecting live delivery later",
    "      setText('emailPreviewModeNote', es ? 'No se envió ningún correo.' : 'No email was sent.');",
    "      setText('emailPreviewModeNote', (es ? 'No se envió ningún correo. ' : 'No email was sent. ') + L(getSavingsPassConfig().demoDeliveryNote));",
    A31],
  ["A3.1 synthesis (ruling 6): the take-home preview disclosure reverts to the retired wording",
    "          : \"Preview: email delivery isn’t connected yet.\";",
    "          : \"Preview mode: live email delivery isn't connected yet.\";",
    A31],
  ...
  ["A3.1 Results: the support sentence returns to the specialist screen",
    ...
    "      if (resultsSubhead) { resultsSubhead.textContent = 'Begin with the first match, then compare comfort in person.'; resultsSubhead.hidden = false; }",
    A31],
  ["A3.1 Results: the retired details cue returns to the hero card",
    "        +       compareBtn\n        +       tryBtn\n        +       saveBtn",
    "        +       '<button type=\"button\" class=\"noct-card-details\">View match details</button>'\n        +       compareBtn\n        +       tryBtn\n        +       saveBtn",
    A31],
  ["A3.1 Results: the decode hold widens back to the first img in the container",
    "        var hero = document.querySelector('#topPickContainer img.noct-toppick-photo');",
    "        var hero = document.querySelector('#topPickContainer img');",
    A31],
  // Product-proof slice 1 (owner decisions 2026-09-02): the reversible
  // finalist, Undo, the replacement notice and the Save toggle.
  ["product-proof: the finalist toggle stops clearing (a second tap re-chooses)",
    "      if (window._favoriteMattressId === mattressId) {\n        window.clearFinalist(mattressId);",
    "      if (false) {\n        window.clearFinalist(mattressId);",
    A31.concat(PLAN)],
  ["product-proof: clearing finalist status also un-saves the mattress",
    "      window._favoriteMattressId = '';\n      window._finalistUndo = null;\n      if (typeof window._repaintFinalistControls === 'function') window._repaintFinalistControls();\n    };\n    // The reversible control",
    "      window._favoriteMattressId = '';\n      window._finalistUndo = null;\n      window._savedPicks = (window._savedPicks || []).filter(function(p) { return p.id !== mattressId; });\n      if (typeof window._repaintFinalistControls === 'function') window._repaintFinalistControls();\n    };\n    // The reversible control",
    A31.concat(PLAN)],
  ["product-proof: Undo restores by writing the id directly (bypassing the producer's save invariant)",
    "      window._finalistUndo = null;\n      window.chooseFinalist(u.previous);",
    "      window._finalistUndo = null;\n      window._favoriteMattressId = u.previous;",
    A31],
  ["product-proof: the replacement notice stops naming the finalist it would replace",
    "        text.textContent = t('drawer.finalist_replaces', { name: mattressNameById(fav) });",
    "        text.textContent = t('drawer.finalist_replaces', { name: '' });",
    A31],
  ["product-proof: the drawer Save stops un-saving (add-only again)",
    "      var wasFinalist = window._favoriteMattressId === mattressId;\n      window._toggleSavePick(mattressId);",
    "      var wasFinalist = window._favoriteMattressId === mattressId;\n      if (!(window._savedPicks || []).some(function(p) { return p && p.id === mattressId; })) window._toggleSavePick(mattressId);",
    A31],
  ["product-proof: the Summary finalist control stops agreeing with the drawer (set-only again)",
    "      window.toggleFinalist(mattressId);\n      renderHf2();",
    "      window.chooseFinalist(mattressId);\n      renderHf2();",
    A31.concat(PLAN)],
  ["product-proof: the finalist change is no longer announced",
    "      live.textContent = '';\n      live.textContent = text || '';",
    "      live.textContent = '';",
    A31],
  // --- Product-proof drawer, slice 3 (owner-approved revision 2026-09-02) ----
  // --- Consolidation pass (owner ruling 2026-09-02): the standalone Try block
  //     is retired, the Comfort / Support facts ride on the demonstrations,
  //     and the generic schematic is dormant behind a fail-closed gate.
  ["product-proof: the standalone Try block returns (the trial cue renders above the demonstrations)",
    "      }).join('');\n      if (section) {",
    ...
    A31],
  ["product-proof: the Comfort basis attaches to the second demonstration (bases swap)",
    "        var basis = bases[i];",
    "        var basis = bases[1 - i];",
    A31],
  ["product-proof: the story never collapses (empty block renders)",
    "        storyEl.hidden = !(storyHead || storyBody);",
    "        storyEl.hidden = false;",
    A31],
  ["product-proof: a third proof renders",
    "      }).filter(function(p) { return p.title && p.cue; }).slice(0, 2);",
    "      }).filter(function(p) { return p.title && p.cue; }).slice(0, 3);",
    A31],
  ["product-proof: the Try section stays visible with zero proofs",
    "        section.hidden = proofs.length === 0;",
    "        section.hidden = false;",
    A31],
  ["product-proof: a construction label is invented from quiz features",
    "      return { comfort: L(c.comfort), support: L(c.support) };",
    "      return { comfort: L(c.comfort) || (m.features || []).join(' '), support: L(c.support) };",
    A31],
  ["product-proof: a Support basis is synthesized when the story names no support layer",
    "        labels.support ? { role: t('drawer.basis_support'), text: labels.support } : null",
    "        { role: t('drawer.basis_support'), text: labels.support || 'Support core' }",
    A31],
  ["product-proof: the generic schematic renders by default (the dormant gate opens)",
    "      return enabled && diagram;",
    "      return true;",
    A31],
  ["product-proof: the painter calls the schematic renderer unguarded",
    "      if (constructionSchematicAvailable(m)) {\n        if (window.dfmConstructionRender) window.dfmConstructionRender();",
    "      if (true) {\n        if (window.dfmConstructionRender) window.dfmConstructionRender();",
    A31],
  // --- Corrective pass (owner ruling 2026-09-02): the DOCKED action footer ----
  //     (a sibling below the scroll viewport; never an overlay, never a spacer)
  ["docked footer: the footer falls back inside the scroll viewport",
    "        </div>\n        <!-- Docked product-action footer",
    "        <!-- Docked product-action footer",
    A31_LAYOUT],
  ["docked footer: a sticky overlay is restored on the footer",
    "    .drawer-action-footer {\n      flex-shrink: 0;",
    "    .drawer-action-footer {\n      position: sticky;\n      bottom: 0;\n      flex-shrink: 0;",
    A31],
  ["docked footer: the footer becomes an absolute overlay over the viewport",
    "    .drawer-action-footer {\n      flex-shrink: 0;",
    "    .drawer-action-footer {\n      position: absolute;\n      left: 0;\n      right: 0;\n      bottom: 0;\n      flex-shrink: 0;",
    A31_LAYOUT],
  ["docked footer: a fixed spacer masks an overlap instead of structural separation",
    "    .drawer-scroll { overflow-y:auto; padding:20px var(--drawer-pad-x) 20px; flex:1; min-height:0; }",
    "    .drawer-scroll { overflow-y:auto; padding:20px var(--drawer-pad-x) 180px; flex:1; min-height:0; }",
    A31_LAYOUT],
  ["docked footer: the pillow prompt is copied into the footer",
    ...
    A31],
  ["docked footer: the pillow prompt relocates itself into the footer when it fires",
    "      prompt.classList.add('is-visible');\n      analytics.log('finalist_sleep_system_prompt_shown'",
    "      document.getElementById('drawerActionFooter').appendChild(prompt);\n      prompt.classList.add('is-visible');\n      analytics.log('finalist_sleep_system_prompt_shown'",
    A31_LAYOUT],
  ["docked footer: the replacement notice, Undo and the live region leave the footer",
  ...
  ["title focus: the tracker stops seeing the keyboard (a pointer entry leaks into a later keyboard opening)",
    ...
    "",
    A31_LAYOUT],
  ["title focus: the entry modality is inherited from the previous opening (no recompute on open)",
    "      drawer.setAttribute('data-focus-entry', typeof drawerFocusEntryModality === 'function' ? drawerFocusEntryModality() : 'keyboard');",
    "      if (!drawer.hasAttribute('data-focus-entry')) drawer.setAttribute('data-focus-entry', typeof drawerFocusEntryModality === 'function' ? drawerFocusEntryModality() : 'keyboard');",
    A31],
  ["title focus: the close path stops clearing the entry modality",
    "      drawer.removeAttribute('data-focus-entry');\n",
    "",
    A31],
  ["title focus: programmatic focus on the title is removed",
  ...
  ["A3.1 drawer: the price-tier glyph returns to the drawer name",
    ...
    A31],
  ["A3.1 drawer: the construction chip addresses the customer again on the specialist worksheet",
    ...
    A31.concat(CONS)],
  ["A3.1 Sleep System: the featured frame regains the min-height that overflowed the portrait column (F1)",
    "      aspect-ratio: 2 / 1;\n      /* A3.1 (F1): no min-height beside the ratio - a definite minimum",
    "      aspect-ratio: 2 / 1;\n      min-height: 210px;\n      /* A3.1 (F1): no min-height beside the ratio - a definite minimum",
    A31],
  ["A3.1 Sleep System: the specialist-check mark collapses back into the addressed dash",
    "        var checkNeeded = decision.status === 'confirm';\n        if (checkNeeded) mark = '?';\n        return '<button",
    "        var checkNeeded = false;\n        return '<button",
    A31],
  ["A3.1 Sleep System: the position chips lose their pressed state",
    "                  '\" aria-pressed=\"' + (position.id === selected.id ? 'true' : 'false') + '\"' +\n                  ' data-sleep-action=\"demo-position\"",
    "                  '\" data-sleep-action=\"demo-position\"",
    A31],
  ["A3.1 Sleep System: the Flat ghost stops drawing the comparison",
    "              (selected.id !== 'flat'\n                ? '<span class=\"sleep-system__bed-ghost sleep-system__bed-ghost--head\"></span>",
    "              (false\n                ? '<span class=\"sleep-system__bed-ghost sleep-system__bed-ghost--head\"></span>",
    A31],
  ["A3.1 Sleep System: the sidecar names/statuses become visible text again (the duplicate of the rail)",
    "          '<span class=\"sr-only\"><span class=\"sleep-system__plan-item-name\">'",
    "          '<span><span class=\"sleep-system__plan-item-name\">'",
    A31],
  // --- A3.1 Summary / take-home / Brief (owner directive 2026-09-01) ---------
  ["A3.1: the recap lookup returns the approved implication verbatim (staff imperative back on the shared screen)",
    "      var approved = consultImplication(questionId, optionId);\n      if (!approved) return '';",
    "      var approved = consultImplication(questionId, optionId);\n      if (approved) return approved;",
    ["tests/consultation_summary_check.mjs"]],
  ["A3.1: the recap ignores the approved-implication gate (a blank config entry still renders a noun)",
    "      var approved = consultImplication(questionId, optionId);\n      if (!approved) return '';",
    "      var approved = 'x';\n      if (!approved) return '';",
    ["tests/consultation_summary_check.mjs"]],
  ["A3.1: the NEXT cue stops reading the open Sleep System steps",
    "          if ((st === 'open' || st === 'later') && cues[step.id]) return cues[step.id];\n",
    "", A31],
  ["A3.1: the status block's brand/tier metadata line returns",
    "          void chosenBrand; void chosenLine;\n          if (metaEl) { metaEl.textContent = ''; metaEl.hidden = true; }",
    "          if (metaEl) { metaEl.textContent = [chosenBrand, chosenLine].filter(Boolean).join(' \u00b7 '); metaEl.hidden = false; }",
    PLAN],
  ["A3.1: the Brief's ordinal ring is dropped",
    "            + '<span class=\"noct-profile-priority-ordinal\" aria-hidden=\"true\">' + (i + 1) + '</span>'\n",
    "", A31],
