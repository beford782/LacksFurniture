...
const MUTATIONS = [
  ...
  ["X3: the protection eyebrow reclaims the forbidden superlative",
    "              en: 'Suggested for ' + sleepSystemText(protectionGoalLabel(protectionGoal)),\n              es: 'Sugerido para ' + sleepSystemText(protectionGoalLabel(protectionGoal))",
    "              en: 'Best for ' + sleepSystemText(protectionGoalLabel(protectionGoal)),\n              es: 'Mejor para ' + sleepSystemText(protectionGoalLabel(protectionGoal))", SLEEP],
  ...
  ["F2: a rail step change keeps the previous step's view",
    ...
    "        analytics.log('sleep_system_step_viewed'",
    SLEEP, "index.html"],
  // Codex review of PR #120 (2026-09-11): the protection goal badge and
  // rationale are gated on protectorSupportsGoal(primary, goal).
  ["F2 protection: the goal-support gate is dropped (every viewed protector claims the goal)",
    "        var goalSupported = step.id === 'protection' && protectorSupportsGoal(primary, protectionGoal);",
    "        var goalSupported = step.id === 'protection';",
    SLEEP, "index.html"],
  ["F2 protection: the goal-support gate is inverted",
    "        var goalSupported = step.id === 'protection' && protectorSupportsGoal(primary, protectionGoal);",
    "        var goalSupported = step.id === 'protection' && !protectorSupportsGoal(primary, protectionGoal);",
    SLEEP, "index.html"],
  ["F2 protection: the displaced protector's list tag ignores the goal it supports",
    "                  ? (step.id === 'protection' && protectorSupportsGoal(engineTop, protectionGoal)\n                      ? goalBadge\n                      : engineTop.matched",
    "                  ? (engineTop.matched",
    SLEEP, "index.html"],
