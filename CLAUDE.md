...
- **Conditional answers reconcile in one place**: `partner_disturbance` is skipped for a solo sleeper, and two options (`body_type.different`, `temperature.opposite`) are offered only to a shared bed. Because Review lets the customer edit `partner_sleep` afterwards, the quiz state machine — not any individual consumer — owns the rule: a not-asked question carries the `not_applicable` sentinel, a question that becomes asked loses that sentinel and is asked, a withdrawn option stops being the answer, and the Review screen is unreachable until every asked conditional question has one. See the invariant block below `visibleQuestions()` in `index.html`; pinned by `tests/quiz_reduction_check.mjs` section 7.
