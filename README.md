...
tablet web app: customers take a nine-question sleep quiz (eight displayed steps
for a solo sleeper), get personalized
...
- `data/mattresses.csv` — generated for this deployment by `tools/convert_store_data.py` from `incoming/Lacks_Store_Data.xlsx` (built from `incoming/lacks_mattresses.json`). Edit `incoming/`, then rebuild; never hand-edit the CSV here.
- `data/mattresses.json` — generated by `.\build-data.ps1` from the CSVs (the converter runs it). **Never edit by hand.**
...
- Open this repository or its dedicated Git worktree as the project root; do not
  attach an agent to the parent folder that contains multiple DreamFinder repos.
...
- Use `$dreamfinder-retailer-onboarding` for workbook-to-bundle retailer setup
  and `$dreamfinder-release-readiness` for PR or release evidence.
- Keep one task per coherent outcome. Use separate worktrees for parallel edits;
  parallel read-only audits may follow
  [`agent-workflows/parallel-audits.md`](agent-workflows/parallel-audits.md).
