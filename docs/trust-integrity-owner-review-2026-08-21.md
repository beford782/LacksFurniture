...
in §2.4. The physical packet is complete by your 2026-08-22 attestation. The
decision sheet directly below lists all fourteen remaining decisions with the
exact wording, the options and the lead's recommendation for each.

## Decision sheet — 2026-08-22, after the physical pass: the 14 decisions that remain
...
**Recommended order:** decide #1–#5 in one sitting (they are yours alone and
need no one else), and name yourself in #8 at the same time so #6 and #7 can
be signed; commission #9; then #11 on the resulting head; then #12 → #13 →
#14. Decisions #1–#5 approved as recommended change no bytes, so the physical
pass at `f748f59` carries forward unchanged.
