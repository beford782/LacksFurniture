...
pairs for `s4/s2/s8/b4/b6.reason_firm`, `s6.reason_medium`,
`s7/b5.reason_plush`; Decision B, separately: the bounded
matchReasons-only fixture amendment; routing confirmations 1–8
recorded), authored at `incoming/lacks_mattresses.json` and flowed through
the canonical lineage with no pipeline change; the Phase 1 regression
fixture moved in exactly 86 `matchReasons` cells (cumulative 138) with
every other pinned path byte-identical and the hash moved in the same
diff; the suite pins all twelve pairs byte-exact with per-slot removal
mutations and unauthorized-axis/model probes. Spanish **provisional**
(native claim-equivalence review still required before any activation);
`reasons_es` has no reader; the reason surfaces stay omitted under PR #63;
`index.html` byte-identical; no engine source, scoring, ranking,
recommendation or rendered behavior changed; no cooling, medical,
quantified, price, durability, patent or origin claim and no
product-to-product, quantified, superiority, price-positioning or
market-position comparison introduced. **Item 1.3 stays ◐** — twelve of
79 slots do not unblock the rendered-output gate, and no Gated, Proceeds
or Exit clause moved; every other phase and item status is unchanged;
pricing remains dark and 2.2 is untouched. Record:
`docs/catalog-reason-tranche-2-2026-08-29.md`. The previous revision,
earlier the same date, recorded the **Item 1.3 first dark reason-authoring
...
`fc61a262d67d7c656a32f4f735fcebfd98ad6ce0` + `f6267e411979a93854585b7d1a972e63eedbf0b3`;
merge, not squash or rebase; source branch `claude/phase2-1a-pricing-contract`
preserved; merge tree `c407802…` byte-identical to the reviewed head).
Pre-merge CI 33134459093 succeeded at the exact reviewed head `f6267e4`;
post-merge CI 33135513855 and Pages 33135513339 both succeeded at exactly
`95935ab`; nine served artifacts returned HTTP 200 byte-identical to the
merged blobs (`index.html`, `data/store-config.json`, `data/quiz.json`,
`data/mattresses.json`, both dictionaries, `data/allowed-hosts.js`, the demo
`store-config.json`, `incoming/lacks_pricing.json`); served `index.html` blob
`ee39112…` unchanged from the `90ecd04` baseline (the slice touched no
application markup); served pricing DARK — `enabled:false`,
`displayEnabled:false`, all five surfaces false, `products`/`formulas` empty,
every business policy explicitly unapproved; served `gasUrl` blank; the
Daybreak promotions envelope inert and byte-untouched. **The reconciliation
baseline advances from `90ecd04` to `95935ab`** — an application-byte merge
(`data/store-config.json` is consumed at runtime). **Owner ruling recorded
...
`exactPromotionsEnabled:true` formula cross-gate are recorded as superseded
and re-point in 2.1b; the shipped all-dark production state satisfies both
the old and the new model, so nothing merged is invalidated. Item 1.4
unchanged (⏳, deferred matrix); standing gates unchanged: showroom NO,
Spanish provisional, forced-colors before showroom, live email off with
`gasUrl` blank, `Code.gs` undeployed. The previous revision, 2026-08-27,
recorded **Phase 2.1 opened** by owner directive after
...
(the item 1.4 merge close-out) was documentation-only and did not advance it;
PR #67 changed application bytes (`index.html` and the generated demo bundle)
and advanced it to `90ecd04`.)*
...
Slice 5, and its block sits after item 1.7. **(5) Slice 5 — the Sleep Plan
...
*(2026-08-29, later the same day — dark reason-authoring tranche 2,
**implemented**. **Item 1.3 remains ◐**; this entry closes no clause and
lifts no gate. Record: `docs/catalog-reason-tranche-2-2026-08-29.md`.

**What was authorized (owner, 2026-08-29).** Decision A — the exact eight
EN/ES pairs for `s4.reason_firm`, `s2.reason_firm`, `s8.reason_firm`,
`s6.reason_medium`, `s7.reason_plush`, `b4.reason_firm`, `b6.reason_firm`
and `b5.reason_plush` (the evidence record's §12 "additional candidates",
each live-verified in a browser session at 2026-08-30 00:29–00:32 UTC; the
optional s8 "Euro Top" wording not approved); Decision B — separately, the
bounded matchReasons-only amendment of the Phase 1 regression fixture; and
routing confirmations that provisional paired Spanish may merge while
non-rendering with native review still required before activation, that
the eight comfort-level facts are outside the unresolved cooling/legal
route for this bounded dark tranche, that no 3.1 work, no customer
rendering and no language-aware routing is authorized, that the item stays
◐ with its rendered-output gate locked, and that no additional model,
axis, claim, optional wording, cleanup, refactor or governance amendment
is included.

**What shipped.** Eight EN reasons at the top level of
`incoming/lacks_mattresses.json` and their provisional ES counterparts
under each model's `es` object, flowed through builder → workbook →
validator → converter → `build-data.ps1` with **no pipeline change**
(nothing generated hand-edited; lineage check 10/10). Every string states
the page's comfort-level rating and its ordinary category meaning (Firm:
less give and more pushback at the surface; Extra Firm: a very firm feel
with little give; Medium: a balanced feel between plush and firm; Plush: a
softer feel with more give) — **not** product-performance promises; no
top type, construction, name marker, sleeper, outcome, cooling, medical,
quantified, price, durability, patent or origin claim, and no
product-to-product, quantified, superiority, price-positioning or
market-position comparison; the retirement suites are unamended.

**What did not change.** `index.html` is byte-identical: the reasons
remain non-rendering (drawer and Compare reason surfaces omitted under
PR #63), `reasons_es` still has no reader, and no engine source, scoring,
ranking, recommendation or rendered behavior changed — the only
computed-output movement is the authorized dormant `matchReasons`
amendment: exactly 86 cells (s6.medium 8/10; s4, s2, s8, b4, b6.firm 5/10
each; s7, b5.plush 5/10 each; `s9_empty_defaults` untouched; the tranche-1
52 cells byte-identical; cumulative 138), every other pinned path
byte-identical, `feelWord` retained, reconstruction proven, hash moved
`24dad016…` → `d973fae2…` in the same diff; the suite now pins all twelve
pairs byte-exact with per-slot removal mutations and unauthorized-axis /
thirteenth-model probes (185/0). The Spanish pass of the fixture holds the
English text (recorded, not approved).

**State after this tranche.** Twelve of 79 per-feature slots populated
(both languages); 67 blank; the 16 case-fold-dead slots stay dead and
unauthored (3.1 untouched); retired models, cooling fields and default
reasons untouched. **Twelve populated slots do not constitute the Gated
clause's "populated per-feature catalog reason content", and the
rendered-output gate stays locked by ruling regardless.** The Gated and
Proceeds block below is unchanged — neither narrowed, widened, weakened
nor closed — and both Exit assessments remain outstanding. Standing
restrictions unchanged: Spanish provisional; native-Spanish reviewer
unnamed; showroom NO; the 1.4 matrix deferred and blocking; `gasUrl`
blank; pricing dark; 2.2 untouched.)*
...
*(Amended 2026-09-14, PR #122: the step order is now base → pillow →
protection. On Blake's instruction the view layer combines the engine's
adjustability and support groups into one Base step; the engine's selection,
ranking, grouping and hero rule are unchanged — `readSleepSystemGroups()` still
returns its four groups and the Phase 1 output-regression fixture is
byte-identical.)*
...
1. the steps remain in their governed order — base, pillow, protection —
   walked from a single table *(amended 2026-09-14, PR #122: previously four
   steps, adjustability, support, pillow, protection; the Base step composes
   the engine's adjustability and support groups)*;
2. every recommendation presents the customer benefit BEFORE the product
   distinction, in DOM order and in visual weight, on every step in both
   languages;
...
at 100% a three-across rail; at 200% (834 → 417 / 1194 → 597 CSS px) a
two-column rail (Base and Pillow, then Protection) with no sideways rail scroll
and no page horizontal scroll *(amended 2026-09-14, PR #122: previously
four-across and 2×2)*. Drive the named states, not merely the three steps:
base decide-later; base keep-current; base foundation → adjustable base (the
height choice clears); pillow aligned → selected, whose 3-line status must wrap and
...
chips (the top-row chips in particular at 597×374 — the Support chip before
the 2026-09-14 combined base step, the Pillow chip since) and the sticky footer over
...
findings evaluated and resolved; (d) **merge is NOT yet authorized** *(granted later the same day, after the PLAN-01 focused retest — see the merge record below)*; (e)
...
*(2026-08-23 — MERGED AND DEPLOYED. PR #53 merged 2026-08-23 15:05Z as
merge commit `f27f879` (parents `ed9374b` + `cd1f1ef`; merge, not squash;
branch `claude/nocturne-slice5-sleep-plan` kept at `cd1f1ef`; the merge tree
is identical to `cd1f1ef`). Post-merge CI run 32647462080 and Pages run
32647461216 both succeeded at exactly `f27f879`; the public preview serves
`index.html` with SHA-256
`91C267E81FF20FCB1F1EDC350E167ABB11568E58B1CEEF6FAFD841E71F306591` — the
`cd1f1ef` blob — and `store-config`, `quiz`, both dictionaries, `mattresses`,
`allowed-hosts`, `manifest` and the demo index all hash-identical to
`cd1f1ef`; a headless render at 1194×748 and 834×1108 reached Welcome with
its Start button, one heritage line and the data-use sentence, advanced to
the quiz, no unauthorized-domain screen, no data-error overlay, zero console
errors, zero failed requests; served `gasUrl` blank; the demo route answers
200. **Between the owner-attested `466a474` and the merged `cd1f1ef`:**
C8 (`9b551c8`) fixed two external-review P2s — the recommended starting
point's tier-and-position line rendered blank because `showResults()` never
stamps a `tier` on its bucket entries (the fallback now carries the known
tier on a shallow copy), and the Consultation Summary's "Chosen ✓" control
toggled the finalist off (it now routes through the idempotent
`chooseFinalist()`); C9 (`0cc2d0c`) retired the Summary's stale "Saved
finalists are sent" hint for the dictionary pair `hf2.saved_picks_hint`;
C10 (`cd1f1ef`), **owner-authorized 2026-08-23**, changed the "Comfortable
elevation" testing prose from "Try the finalist flat, …" to "Try the
mattress flat, …" (ES provisional "el colchón"), with the Phase 1 output
fixture regenerated by the established tool (exactly two strings moved,
sha `4aa0e7ad…` → `e7e5c27d…`) and the Sleep Brief `572d405` pin hand-edited
in its eight prose occurrences per the 2026-08-10 precedent (sha
`75b4d244…` → `0dbd354c…`), both pins moved in the same diff; ruled
literals pinned independently in source and fixture; a vocabulary
invariant over every emitted priority; and a no-finalist Plan regression
built from the real producer strings. Four external-review threads, all
fixed, none waived; CI run 32646265947 green at `cd1f1ef`; mutation sweep
410/410. **PLAN-01 focused retest, owner-attested PASS at `cd1f1ef`**
(2026-08-23, browser verification accepted by Blake Ford, on the retained
dry-run mirror `df-phase1-dryrun@d349458` whose served `index.html` was the
`cd1f1ef` blob): RT-01 the recommended starting point shows its
...
same finalist selected. No requirement in this document or the Slice 5
suites mandates a direct "Back to Plan" control on the Summary; whether one
should exist is an owner wording/navigation decision for a later item.
**Merge authorized by Blake Ford 2026-08-23** (merge commit, branch kept).
Standing rulings unchanged: showroom use NOT authorized; the rendered
Windows forced-colors check was waived for this preview merge only and
remains REQUIRED before showroom authorization; native-Spanish review
deferred to the consolidated end-of-development language pass — Spanish
remains provisional. Item 1.7 moves 🔨 → ✅; the exit below is met on
`main`.)*

*(2026-08-23, later the same day — a shipped layout defect the merge verification did not catch, repaired by a hotfix PR ahead of Slice 6. A read-only Slice 6 discovery pass rendered the deployed `d6ca237` preview headlessly (Chromium, the app's own functions, `index.html` SHA-256 `91C267E8…F306591` = the `cd1f1ef` blob) and found that `#sleepPlanScreen` had **no CSS rule of its own**: `.screen.active { display: flex }` applied with the flex-ROW default and the dark root tokens, so the Sleep Plan rendered as nine side-by-side columns at 1194×748 and 834×1108; at 834×1108 the document scrolled horizontally (scrollWidth 1169) and the Payment Choice block and the Continue control lay outside the viewport; the Payment Choice headline measured 1.09:1 against its surface. The omission is the hf2 layout override (`#hf2Screen.active { flex-direction: column }`) and the warm work-theme groups, neither of which named the Plan. **The bytes were identical to those of the 2026-08-23 device walk and the PLAN-01 retest; the defect was not reported by either. This entry records that a later rendered verification found it; it does not characterise what the earlier walk observed.** Repair: branch `claude/hotfix-sleep-plan-layout` — the Plan joins hf2's column override and every warm-theme group it lacked, its Continue control is restated on the store primary like the Summary's Send button (the base pairing is 3.67:1 on this theme), no wording, data, scoring, email or navigation change; guarded by eight new static pins in `tests/sleep_plan_check.mjs` (layout, theme membership, hf2↔Plan selector parity, a roster-derived rule that every multi-child screen carries a column override, computed Continue contrast with a negative control), six mutation entries, and the repository's first RENDERED check `tests/sleep_plan_layout_check.py` (headless Chromium on a loopback server at 1194×748, 834×1108, 390×844 and 597×374: computed column layout, no horizontal overflow, theme parity with the Summary, title / headline / Continue contrast ≥ 4.5:1, Continue Tab-reachable) — all red against `d6ca237`, green on the branch, wired into CI with pinned Playwright. Merge, deployment and the device matrix for these bytes remain owner decisions; showroom authorization unchanged (NO).)*
...
**Merged — recorded 2026-08-28.** PR #69 merged 2026-08-28 02:18:19Z as merge
commit `95935abe8493f22e99b7881e1abdb7c7f4deb5cd` (parents
`fc61a262d67d7c656a32f4f735fcebfd98ad6ce0` +
`f6267e411979a93854585b7d1a972e63eedbf0b3`; merge, not squash or rebase;
source branch `claude/phase2-1a-pricing-contract` preserved; merge tree
`c407802…` byte-identical to the reviewed head, which carries all three slice
commits `e083b44` → `20645ff` → `f6267e4`). Pre-merge CI 33134459093 succeeded
at the exact reviewed head; post-merge CI 33135513855 and Pages 33135513339
both succeeded at exactly `95935ab`. Nine served artifacts returned HTTP 200
byte-identical to the merged blobs; served `index.html` blob `ee39112…` is
unchanged from the `90ecd04` baseline (no application markup touched); the
served `store-config.json` carries the pricing block exactly as authored —
`enabled:false`, `displayEnabled:false`, all five surfaces false,
`products: []`, `formulas: []`, `purchaseAssessment.policy:
"runtime-transaction-amount"`, freshness/sourcePolicy/presentation all
explicitly unapproved; served `gasUrl` blank; the Daybreak promotions envelope
inert (`activeScenario: null`, `scenarios: {}`) and byte-untouched; production
`financing.exactPromotionsEnabled` false as shipped. The reconciliation
baseline advances from `90ecd04` to `95935ab` — an application-byte merge.

**Owner ruling — recorded 2026-08-28 (approval timing).**
Blake ruled: **final business and legal approval occurs only at the end of the
project, immediately before live deployment. It does not block dark
development, provisional data, internal computation, previews, testing, or
completion of earlier roadmap items.** That timing statement is the owner's
direct ruling. The numbered model below is the Phase 2 governance revision
recorded under it — **implementation governance, not quoted owner speech**:
the existing native-review, MAP-clearance and other operational controls
remain activation requirements where already specified, and their precise
timing is implementation governance.

**The revised Phase 2 governance model — technical verification separated
from live-output approval:**

1. `pricing.enabled` may support the dark resolver without final business,
   legal, native-review or MAP approval.
2. Technical source, identity, freshness and formula-integrity checks still
   fail closed. Dark verification still requires **governed source identity,
   host allowlisting, freshness and integrity** — technical source
   authorization is NOT deferred to deployment.
3. Final business and legal approval — and the existing native-review and
   MAP-clearance activation requirements, where already specified — gate
   `displayEnabled`, enabled surfaces and live deployment **only**.
4. `exactPromotionsEnabled: false` prevents live exact-term **output**; it
   must not prevent **validation** of a dark formula artifact.
5. Provisional material must remain explicitly labelled pending / not
   approved. It **may** appear in clearly identified NON-SHIPPING preview or
   test fixtures and harnesses; it must **not** appear in the shipped
   customer experience, production email payloads, analytics, persistence,
   or live deployment, and must never be represented as approved. "Must not
   render" means the shipped/live customer surfaces — it is not a ban on
   controlled previews.
6. `displayEnabled` and every surface remain false throughout Phase 2.1 (the
...
governance model above and re-point in slice 2.1b: (a) `validate_pricing`
requires ownership, MAP clearance and approved ES review **when `enabled`** —
under the split those approvals re-bind to the activation set
(`displayEnabled`, enabled surfaces, live deployment), remaining activation
requirements exactly where already specified, while `enabled` admits the dark
resolver on the technical checks alone; (b) a formula artifact is admitted
**only under
`exactPromotionsEnabled: true`** — under the split that flag gates live
exact-term output only, and a dark formula artifact validates on its technical
integrity (verified/fresh/allowlisted financing configuration and plan under
the FINANCING source policy, `calculationMode` match, complete named inputs,
verified cadence) with `exactPromotionsEnabled` false. Until 2.1b lands, the
merged validator remains the enforced contract; the shipped all-dark,
all-empty, all-unapproved production state satisfies both the old and the new
model, so nothing merged is invalidated and no CI lock weakens. Every
fail-closed technical check (per-entry `verifiedAt` + `sourceUrl` +
`verifiedBy`, dual host-allowlist agreement, injected-clock freshness, integer
minor-unit money, per-entry twelve-field clearance scope, strict nested-key
...
(1) price availability/resolution; (2) financing calculation availability,
including *quote-only*; (3) purchase-threshold status from the explicit
...
`tests/scoring_isolation_check.mjs`, DOM silence on shipped customer
surfaces, and **no provisional material in production email payloads,
analytics or persistence** (extending the payload pins); (ii)
the validator re-binding described above — approvals move to the activation
set, `enabled` becomes the technical dark-resolver admission, the formula
cross-gate drops its `exactPromotionsEnabled: true` condition while every
other financing-governance check stays fail-closed, and
`tests/exact_promotions_policy_check.mjs` continues to pin that false means no
live exact-term output; (iii) explicit pending / not-approved labelling on
all provisional material — permitted in clearly identified non-shipping
preview/test fixtures and harnesses, pinned out of the shipped customer
experience, production email payloads, analytics and persistence, and never
representable as approved; (iv) **governed
...
proof of the complete dark mechanism on those fixtures with zero price or
...
>
> Phase 2.1 still requires the full fail-closed mechanism even for dark data —
> governed source identity and host allowlisting, freshness and cadence
> control, size identity, validation, and emergency-disable behaviour;
> technical source authorization is not deferred to deployment. Ownership
> approval, final business/legal approval and the activation-time clearances
> (MAP, native review) gate live output at the end-of-project deployment
> stage (the 2026-08-28 ruling and its recorded governance model), and
> nothing in this file supplies any of it — the ring-fence stands in full.
...
**A green static suite does not mean a screen renders.** The Sleep Plan shipped (`f27f879`) with no CSS rule for `#sleepPlanScreen` and rendered as a flex-row of nine columns on the dark theme while every suite and the mutation sweep were green, because no check rendered anything. Found 2026-08-23 by a headless render of the deployed preview during Slice 6 discovery; repaired by the Sleep Plan layout hotfix (see the 1.7 record). Two durable rules follow: a multi-block screen built on another screen's classes needs that screen's layout override and theme groups (now derived from the screen roster by `tests/sleep_plan_check.mjs`), and a visible redesign is verified by a rendered check (`tests/sleep_plan_layout_check.py` is the pattern), not by string presence alone.
