...
**Status:** ready for the owner to execute. **Nothing here is evidence yet** —
...
`c65a437`; `index.html` is unchanged between `12ad950` and the current head, so
they hold for any head that does not touch `index.html` (re-measure otherwise —
§11). The idle-dialog wording is the R5 sentence shipped in `444abe2`.
...
| Served URL | (the mirror URL — see §10 blocker 2; NOT `beford782.github.io/LacksFurniture/`, which serves `main`) |
| Result | PASS / FAIL / PARTIAL (list failing IDs) |
| Evidence location | directory path per §8, outside `git` |
...
Setup: iPad in its mount, landscape. Open the served URL fresh (close any other Safari tab on it). Language EN. Do not use a keyboard unless a step says so. Tap with a finger; the app is touch-first.
...
| IPAD-L-EN-02 | Heritage line | One small-caps heritage line above "The sleep store…" headline | Exactly one; footer shows no second line (§2) | | same shot |
...
| IPAD-L-EN-07 | Answer Q1, Next | Tap "Back pain / aches" (any option), tap Next → | Q2 headline top edge sits just below the progress bar (emulation 138 px from the top of the viewport). Eyebrow "QUESTION 2 · OF 10" visible. **No part of Q1 visible.** The page did not "jump" mid-animation | | |
...
| IPAD-L-EN-10 | Partner path | Pick "My partner", Next | Q4 "Does your partner's movement wake you up?" appears (this question exists ONLY on the partner/family path); help "The more movement wakes you, the more it shapes your matches and what we suggest testing." | | |
| IPAD-L-EN-11 | Q4→Q5 | Pick any, Next | Q5 sleep position; help "This helps us favor pressure relief, support, or a responsive feel." | | |
| IPAD-L-EN-12 | Q5→Q6 | Pick "Side", Next | Q6 body type; help unchanged: "This helps us account for cushioning, support, and durability." Note the fifth option "We're different sizes" is present (partner path only) | | |
| IPAD-L-EN-13 | Q6→Q7 | Pick any, Next | Q7 temperature; help "If you sleep hot, we favor cooling features in your matches." Fourth option "We're opposites" present (partner path) | | |
| IPAD-L-EN-14 | Q7→Q8 firmness | Pick "Hot", Next | Q8 slider question; help "No wrong answer here, just slide to the feel you prefer." This screen is taller than the viewport: scroll to the bottom to reach Next | | |
...
| IPAD-L-EN-18 | Forward again | Tap Next → on Q9; tick "Snoring" and "Nighttime reflux" on Q10; tap "Review My Answers →" | Review screen: "ALMOST THERE", "Quick review", then the sentence in IPAD-L-EN-19 | | |
...
| IPAD-L-EN-20 | **Edit from Review** | Tap "Edit" on the third row (Who's sharing the bed) | The quiz reopens on Q3 with its headline at the top (≈138 px), "QUESTION 3 · OF 10", your previous answer selected. Tap Next repeatedly back to Review — each question lands at the top | | |
...
| IPAD-L-EN-31 | **Sleep System containers after Restart — the observable proxy** | As a second customer: Q1 any, Queen, **"Just me"**, Side, any, "Comfortable", slider default, Q8 tick **"No Major Issues"** only, Q9 tick **"None of these"** only → Review → Sleep Brief → Results → "Build Your Sleep System →" | **No** line mentioning back pain, snoring or reflux anywhere on the Sleep System screen; no adjustable-base "suggested first because…" line. **What this proves and does not prove:** without devtools the owner cannot see the emptied containers at the moment of Restart — the screen is rebuilt from the new answers before it is shown again. This step proves no first-customer prose survives into the second customer's Sleep System *as seen*. The clearing at Restart itself is proven by `tests/session_safety_check.mjs` (sentinel seeded into every `SESSION_CONTENT_IDS` entry, including the four Sleep System containers) — a code result, recorded as such | | screenshot |
...
| IPAD-L-EN-36 | Evidence | Screenshots for -03, -15, -16, -17, -19, -22, -23, -25, -27, -30, -31, -35 renamed per §8 | All present in the evidence directory | | |
...
| IPAD-P-EN-02 | Q1→Q7 | Headline at ≈122 px each time; no overlap between the utility bar (top right) and the eyebrow/progress row | | |
...
| IPAD-P-EN-06 | Review sentence | Wraps naturally (one or two lines), not clipped at the right edge | | screenshot |
| IPAD-P-EN-07 | Tier note wraps | "Match strength is relative within each tier" fits on one line at 738 px wide (emulation); if it wraps, it wraps cleanly | | screenshot |
...
| ID | Step | Pass criterion | Result | Evidence |
|---|---|---|---|---|
| SOLO-EN-01 | Q3 "Just me" → Next | **Q4 is skipped**: the next screen is sleep position and the eyebrow reads "QUESTION 4 · OF 9" (total 9, not 10). No "partner's movement" question appears | | screenshot |
| SOLO-EN-02 | Body type / temperature | Q5 body type shows **four** options (no "We're different sizes"); Q6 temperature shows **three** (no "We're opposites") | | |
| SOLO-EN-03 | **Back across the skip** | On sleep position tap ← Back | Lands on "Who's sharing the bed with you?" (Q3 · of 9) with "Just me" selected — not on the hidden partner-movement question; headline at the top | | screenshot |
...
| SOLO-ES-01…06 | Same in Spanish ("Pregunta 4 · de 9", "Solo yo") | | | |
...
| VO-EN-06 | On sleep issues double-tap two options, then "Next" | `h2` | "Anything else going on that affects your sleep?" once; then swipe: help line, "Pick up to 3", options | | | |
...
| VO-EN-10 | Swipe to the top-right "Español" button, double-tap | `restoreLanguageFocus()` → the Español button itself; `<html lang>` becomes `es` synchronously | **"Español, selected"** (or equivalent) once; **no** headline, **no** "Sleep quiz". Subsequent swipes read the same question in Spanish, in the Spanish voice if one is installed (VoiceOver follows `lang="es"`) | The headline or screen name spoken on a language switch; two announcements of the button; the English voice reading Spanish when a Spanish voice is installed (record; not a trust fail) | | |
| VO-EN-11 | Double-tap "English" to return; Next to Review; double-tap "See my sleep signature" | reveal overlay `role="status"` first, then `h1#profileName` | The reveal's status text once (pre-existing), then the profile name as a heading once | Profile name twice | | |
...
| VO-EN-13 | Swipe to "Restart" (top right), double-tap | `h2#sessionSafetyTitle` inside `role="alertdialog"` (`aria-labelledby` title, `aria-describedby` body); everything else made `inert` | "Start a new customer?" and the body "This clears the current answers, mattress selections, and Sleep Plan." — each once, title first (the description may follow the title automatically or on the next swipe). Swiping right reaches only "Keep this session" and "Start new customer"; the page behind is unreachable | Title twice; body never reachable; swiping escapes to the results page behind | | |
| VO-EN-14 | Double-tap "Keep this session" | the Restart button (the stored opener) | "Restart, button" once; no screen name | "Your matches" spoken as well | | |
...
| VO-EN-17 | Double-tap "Continue this session" | the stored opener if any; otherwise the active screen container | If nothing was focused before the dialog (the normal idle case): the screen name ("Your matches") once. If "Restart" was focused: "Restart, button" once | Two announcements; nothing | | |
...
| VO-ES-01…18 | Repeat with Spanish selected on Welcome before starting; expected strings per §2/§4 ("Cuestionario de sueño" for the quiz region, "Revisión rápida", "Tus opciones", "¿Empezar con otro cliente?", "¿Sigues comparando?", "Quedan N segundos" never spoken, "Esta sesión se borrará en unos N segundos." once) | | With a Spanish voice installed, the Spanish voice reads every screen; after the wipe the voice returns to English | | | |

What a VO failure means: a duplicate or a missing announcement here is recorded and handed to the lead for a ruling; the 2026-08-12 ruling means it is not automatically a merge blocker, but R7 requires the pass to have been *run* and its findings recorded before #54 leaves draft.
...
3. **Accelerator — layout/copy only, not timing evidence.** Settings → General → Date & Time → turn off "Set Automatically" → move the clock forward 6 minutes while Safari is open on the kiosk page. The ticker compares wall clock each second, so the warning opens within ~1 s with a fresh 5-minute meter; moving the clock another 6 minutes forward produces the expiry wipe. Restore "Set Automatically" immediately afterwards. Use this only for retests of wording/layout (e.g. after a dictionary change), and record "clock-advanced" in the Notes column — the real-wait route stays the timing evidence.
...
- **Also record** the served `index.html` SHA-256 (the lead computes it from the mirror, §11 blocker 2) so the observation is tied to bytes, not to a branch name.
