...
def is_promotional_presentation(plan) -> bool:
    ...
    WHAT THIS IS PINNED TO, PRECISELY:
      ...
      * It is pinned to `_plan_group(plan) == "promotional"` in
        tools/validation.py for every plan JSON can express, malformed ones
        included: a non-string scenario is coerced to absent (falling through
        to `kind`) and any truthy scenario string, whitespace-only or
        unrecognised, is NOT promotional. validation.py's self-test pins the
        two equal across that whole space. Mirroring rather than importing
        keeps this module free of repo dependencies, which is what lets the
        workbook builder, the validator and the tests share it without an
        import cycle.

        Outside that space the two are not equal and are not claimed to be:
        this function ANSWERS for any object, while _plan_group() raises
        TypeError on an unhashable `kind` (its `kind in <set>` test hashes).
        That is a pre-existing robustness gap in validate_financing's wider
        input handling, not a classification disagreement — see the
        follow-up note in the amend report.
...
def apr_in_domain(value) -> bool:
    ...
    if type(value) is int:
        return APR_MIN <= value <= APR_MAX          # exact; no float conversion
    if type(value) is float:
        return math.isfinite(value) and APR_MIN <= value <= APR_MAX
    return False
